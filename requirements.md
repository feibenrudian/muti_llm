...
- 详情页（单次请求 Trace 视图，按时间线展示）：
  ...
  5. **换裁判重新聚合**：裁判卡片内以**下拉菜单**切换裁判模型（选项仅限本次请求的成员模型 + 裁判模型，避免全量模型列表过长），默认选中当前裁判模型——选中已有结果的模型直接展示其输出；选中未生成过结果的模型立即**流式**重跑：复用该 trace 已存的成员答案、按 Pipeline 当前模板重新渲染裁判 Prompt，只重调裁判不重调成员，输出实时增量显示；多模型可并行重跑互不干扰，未完成的在切换回来后继续实时显示；失败尝试保留错误信息且可重新发起。重跑以 `judge_rerun` 行追加保存（含失败尝试），**不回写原始记录**（最终响应与状态保持客户端当时实际收到的结果）。
